...
(() => {
  ...
  function openOrderSuccess(orderNumber,waText){openDrawer(`<button class="close" onclick="closeOverlay()">×</button><div class="success-box"><div class="success-icon">✓</div><h2>Order Confirmed!</h2><p style="margin-bottom:20px;font-size:14px;color:#555">Your order has been received by PowerRun Industries.</p><div style="background:#f7f7f7;border:2px solid var(--orange);border-radius:10px;padding:20px;margin:20px 0;text-align:center"><div style="font-size:12px;color:#777;margin-bottom:6px">ORDER ID</div><div style="font-size:24px;font-weight:900;color:var(--orange);font-family:monospace;word-break:break-all">${esc(orderNumber)}</div></div><p style="font-size:13px;color:#555;margin:15px 0"><b>What's next?</b> We will contact you within 24 hours to confirm payment and delivery details. Save your Order ID for reference.</p><div style="display:flex;gap:8px;flex-wrap:wrap"><button class="btn orange" onclick='location.href="https://wa.me/${PR_WA}?text="+encodeURIComponent(${JSON.stringify(waText)})' style="flex:1">📲 WHATSAPP CONFIRMATION</button><button class="outline" onclick="closeOverlay()" style="flex:1">CLOSE</button></div></div>`);}window.openOrderSuccess=openOrderSuccess;
  ...
  async function openAccount(){
    ...
    const rows = (orders || []).map(o => `<tr><td><b>${esc(o.order_number)}</b></td><td>${money(o.total_amount)}</td><td>${esc(o.order_status || 'new')}</td><td>${new Date(o.created_at).toLocaleDateString('en-IN')}</td><td><button class="outline" style="padding:6px 10px;font-size:11px;" type="button" onclick='viewOrderDetail(${JSON.stringify(o.id)})'>View</button></td></tr>`).join('');
    openDrawer(`<button class="close" type="button" onclick="closeOverlay()">×</button><h2>My Orders</h2><p class="small-note">Signed in as ${esc(session.user.email || '')}</p><div class="table-scroll"><table><thead><tr><th>Order</th><th>Total</th><th>Status</th><th>Date</th><th>Action</th></tr></thead><tbody>${rows || '<tr><td colspan="5">No orders are associated with this account yet.</td></tr>'}</tbody></table></div><button class="outline" type="button" onclick="accountLogout()">SIGN OUT</button>`);
  }
  async function viewOrderDetail(orderId){
    const { data: order, error } = await sb.from('orders').select('*').eq('id', orderId).single();
    if (error || !order) { toast('Order not found.', 'error'); return; }
    const { data: items } = await sb.from('order_items').select('*').eq('order_id', orderId);
    const itemsHtml = (items || []).map(x => `<div class="summary-row"><span>${esc(x.product_name || '')} × ${x.quantity}</span><b>${money(x.total_price || 0)}</b></div>`).join('');
    openDrawer(`<button class="close" type="button" onclick="closeOverlay()">×</button><h2>Order ${esc(order.order_number)}</h2><div class="order-summary"><b>Order Details</b><div class="summary-row"><span>Status</span><b>${esc(order.order_status || 'new')}</b></div><div class="summary-row"><span>Payment</span><b>${esc(order.payment_status || 'pending')}</b></div><div class="summary-row"><span>Total</span><b>${money(order.total_amount)}</b></div><div class="summary-row"><span>Placed on</span><b>${new Date(order.created_at).toLocaleDateString('en-IN')}</b></div></div><div class="order-summary"><b>Items</b>${itemsHtml}</div>${order.tracking_number?'<div class="order-summary"><b>Tracking</b><div class="summary-row"><span>Tracking Number</span><b>'+esc(order.tracking_number)+'</b></div><div class="summary-row"><span>Courier</span><b>'+esc(order.courier_partner||'—')+'</b></div><div class="summary-row"><span>Expected Delivery</span><b>'+esc(order.expected_delivery_date||'—')+'</b></div></div>':''}`);}
  ...
  async function editProduct(id){await loadCategories();await loadProducts(true);if(!id&&dbProducts.length>=25){toast('Product limit reached (25/25). Delete a product to add a new one.','error');return}editingProductId=id||null;selectedNewFiles=[];const p=id?dbProducts.find(x=>String(x.id)===String(id)):{name:'',category_id:dbCategories[0]?.id||'',price:'',mrp:'',sku:'',short:'',description:'',specs:'',featured:false,new:true,visible:true,_images:[]};editingExistingImages=[...(p._images||[])];const cats=dbCategories.map(c=>`<option value="${esc(c.id)}" ${String(c.id)===String(p.category_id)?'selected':''}>${esc(c.name)}</option>`).join('');document.getElementById('adminContent').innerHTML=`<div class="admin-heading"><div><h1>${id?'Edit':'Add'} Product</h1><p>Maximum 5 product images per product • 25 products max.</p></div></div><div class="admin-table"><form class="form" onsubmit="saveProduct(event)"><label>Product Name *<input id="f_name" required value="${esc(p.name)}"></label><label>Category *<select id="f_cat" required>${cats}</select></label><label>Selling Price (₹)<input id="f_price" type="number" min="0" step="1" value="${esc(p.price??'')}" placeholder="Leave blank for Price on request"></label><label>MRP / List Price (₹)<input id="f_mrp" type="number" min="0" step="1" value="${esc(p.mrp??'')}" placeholder="Optional — shown as a strikethrough price with discount %"></label><label>SKU<input id="f_sku" value="${esc(p.sku)}"></label><label>Short Description<textarea id="f_short">${esc(p.short)}</textarea></label><label>Full Description<textarea id="f_desc">${esc(p.description)}</textarea></label><label>Specifications<textarea id="f_specs">${esc(p.specs)}</textarea></label><div><b>Existing Images (${editingExistingImages.length}/5)</b><div class="thumbs" id="existingThumbs">${editingExistingImages.map((x,i)=>`<div class="thumb-wrap"><img src="${esc(x.url)}" alt="Product image"><button type="button" onclick="removeExistingImage(${i})">×</button></div>`).join('')||'<small>No images uploaded.</small>'}</div></div><div class="drop"><label>Upload new images (${Math.max(0,5-editingExistingImages.length-selectedNewFiles.length)} remaining)<input id="f_files" type="file" accept="image/jpeg,image/png,image/webp" multiple onchange="selectFiles(this)"></label><small>JPG/PNG/WebP max 10MB each. Up to 5 total images per product.</small><div class="thumbs" id="newThumbs"></div></div><label><input id="f_new" type="checkbox" ${p.new?'checked':''}> Mark as NEW</label><label><input id="f_featured" type="checkbox" ${p.featured?'checked':''}> Featured Product</label><label><input id="f_visible" type="checkbox" ${p.visible?'checked':''}> Visible on website</label><div><button class="admin-btn" id="saveProductBtn" type="submit">SAVE PRODUCT</button> <button type="button" class="admin-btn gray" onclick="adminProducts()">CANCEL</button></div></form></div>`;}
  window.editProduct=editProduct;
  function selectFiles(input){const files=[...input.files];const maxAllowed=5-editingExistingImages.length;if(files.length>maxAllowed){input.value='';toast(`Maximum ${maxAllowed} image${maxAllowed===1?'':'s'} remaining for this product.`,'error');return}selectedNewFiles=files.filter(f=>{if(!/^image\/(jpeg|png|webp)$/.test(f.type)){toast(f.name+': Only JPG/PNG/WebP images allowed.','error');return false}if(f.size>10*1024*1024){toast(f.name+': Image too large (max 10MB).','error');return false}return true});if(selectedNewFiles.length!==files.length)return;document.getElementById('newThumbs').innerHTML=selectedNewFiles.map(f=>`<img src="${URL.createObjectURL(f)}" alt="Preview">`).join('');}
